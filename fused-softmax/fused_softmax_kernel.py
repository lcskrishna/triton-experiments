...
def naive_softmax():
    x_max = x.max(dim=1, keep_dim=True)
    z = x - x_max[:, None]
...
@triton.jit
def fused_softmax_kernel(output_ptr, input_ptr, input_row_stride, output_row_stride, n_rows, n_cols,
...
def softmax(x):
    ...
    if is_hip():
        if is_cdna():
            NUM_GPRS = NUM_REGS * 2
        MAX_NUM_THREADS = 1024 #properties["max_threads_per_sm"] 
...
benchmark.run(show_plots=True, save_path='results')
